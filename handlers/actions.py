...
STEAL_COOLDOWN = 1800  # 30 minutes in seconds
    ...
    if victim.is_bot:
        if victim.id == context.bot.id:
            ...
            if random.random() < STEAL_SUCCESS_RATE:
                ...
                await db.add_xp(db_client, thief.id, thief.username, xp_to_add=stolen_amount)
                message = f"🎉 {thief.mention_html()} masterfully swiped {stolen_amount} XP from the bot!"
                logger.info("XP steal success from bot", thief_id=thief.id, amount=stolen_amount)
                await update.message.reply_html(message)
            else:
                await db.add_xp(db_client, thief.id, thief.username, xp_to_add=-STEAL_PENALTY)
                message = (
                    f"🚓 Oh no! {thief.mention_html()} fumbled the attempt to rob the bot "
                    f"and lost {STEAL_PENALTY} XP in the process!"
                )
                await update.message.reply_html(message)
            return
        else:
            await update.message.reply_text("You can't steal from other bots, they have no pockets to pick!")
            return
