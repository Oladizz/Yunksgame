...
async def test_start_new_game_command(mock_randint, mock_admin_update, mock_context):
    """Test starting a new game via command."""
    mock_admin_update.callback_query = None  # Ensure it's a command-based invocation
...
@pytest.mark.asyncio
@patch('random.randint', return_value=42)
async def test_start_new_game_callback(mock_randint, mock_admin_update, mock_context):
    """Test starting a new game via callback query."""
    # Make sure message is not None for edit_text
    mock_admin_update.callback_query.message = AsyncMock()
    
    await game_guess_number.start_new_game(mock_admin_update, mock_context)

    mock_admin_update.callback_query.message.edit_text.assert_called_once()
    assert "I'm thinking of a number" in mock_admin_update.callback_query.message.edit_text.call_args[0][0]
    assert 'game' in mock_context.user_data
    assert mock_context.user_data['game']['secret_number'] == 42
    assert mock_context.user_data['game']['tries_left'] == 7
