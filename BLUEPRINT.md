...
- [ ] **Unknown Command Handler:** Gracefully handle unrecognized commands.
...
- [ ] **Award XP (`/awardxp`):**
    - [ ] Admin-only command.
    - [ ] Requires replying to a user and specifying an amount.
    - [ ] Awards XP to the target user.
- [ ] **End Game (`/endgame`):**
    - [ ] Admin-only command.
...
- [ ] **"Rat in the Farm" Game:**
    - [ ] `/farm` command to start a lobby.
    - [ ] Lobby management (join, leave, start game).
    - [ ] Player roles (Farmer, Rat).
    - [ ] Farm locations and damage mechanics.
    - [ ] Game phases: Lobby, Search, Results, Movement, Suspicion, Accusation.
    - [ ] XP awards for winning role (Farmer or Rat).
...
## 6. Utility Functions
- [ ] **`strict_edit_message`:** Function to prevent redundant API calls for message editing.
